...
class NoteAPITest {
    ...
    inner class DeleteNotes {
        ...
        fun `deleting a Note that does not exist, returns null`() {
            Assertions.assertNull(emptyNotes!!.deleteNote(0))
            Assertions.assertNull(populatedNotes!!.deleteNote(-1))
            Assertions.assertNull(populatedNotes!!.deleteNote(5))
        }
    ...
    inner class UpdateNotes {
        ...
        fun `updating a note that does not exist returns false`() {
            Assertions.assertFalse(populatedNotes!!.updateNote(6, Note("Updating Note", 2, "Work", false)))
            Assertions.assertFalse(populatedNotes!!.updateNote(-1, Note("Updating Note", 2, "Work", false)))
            Assertions.assertFalse(emptyNotes!!.updateNote(0, Note("Updating Note", 2, "Work", false)))
        }
    ...
    inner class ArchiveNotes {
        ...
        fun `archiving a note that does not exist returns false`() {
            Assertions.assertFalse(populatedNotes!!.archiveNote(6))
            Assertions.assertFalse(populatedNotes!!.archiveNote(-1))
            Assertions.assertFalse(emptyNotes!!.archiveNote(0))
        }

        @Test
        fun `archiving an already archived note returns false`() {
            assertTrue(populatedNotes!!.findNote(2)!!.isNoteArchived)
            Assertions.assertFalse(populatedNotes!!.archiveNote(2))
        }

        @Test
        fun `archiving an active note that exists returns true and archives`() {
            Assertions.assertFalse(populatedNotes!!.findNote(1)!!.isNoteArchived)
    ...
    inner class SearchMethods {
        ...
        fun `search notes by title returns notes when notes with that title exist`() {
            ...
            Assertions.assertFalse(searchResults.contains("Swim - Pool"))
